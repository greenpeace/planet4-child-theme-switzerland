			...
			<InspectorControls>
				...
				{Array.from({ length: Math.max(tierLabels.length, 5) }).map((_, i) => (
					<div key={i} className="food-quiz__tier-config">
						<h3>{__('Tier', 'planet4-child-theme-switzerland')} #{i + 1}</h3>
						<TextControl
							label={`Label`}
							value={tierLabels[i] || ''}
							onChange={(v) => {
						...
						{(i < 4) ? (
							...
							<PanelRow>
								<span><strong>{__('Upper threshold: Infinite', 'planet4-child-theme-switzerland')}</strong></span>
							</PanelRow>
						)}
					</div>
				))}
							...
							{drinks.map((d, i) => (
								<div key={i} className="fq-drink-wrapper">
									...
									<div className="fq-drink-controls">
										<button type="button" className="fq-drink-decrease" aria-label="Decrease">−</button>
										<input type="number" min="0" max="5" value="0" data-index="0" className="fq-drink-input" readOnly />
										<button type="button" className="fq-drink-increase" aria-label="Increase">+</button>
									</div>
								</div>
							))}
						</div>
					...
					<div className="food-quiz__tiers-editor">
						...
						<div className="food-quiz__tiers-tabs">
							{Array.from({ length: Math.max(tierLabels.length, 5) }).map((_, i) => (
								...
								>
									{tierLabels[i] || `ResultTier ${i + 1}`} <small>(≤{tierThresholds[i] !== null && tierThresholds[i] !== undefined ? tierThresholds[i] : '∞'})</small>
									</button>
								))}
						</div>
